import { Bell, Crown, GitBranch, Globe, Laptop, Radar, ShieldAlert, Siren } from 'lucide-react'
...
export function KpiTiles({ kpis, coverage }: { kpis: DashboardKpis; coverage: DashboardCoverage }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-2 md:grid-cols-4 xl:grid-cols-8">
      ...
      />
      <KpiTile label="IOC matches" value={fmtNum(kpis.ioc_matches)} tone={kpis.ioc_matches ? 'warn' : 'neutral'} hint="indicators seen in telemetry" icon={<Radar size={13} />} onClick={() => navigate('/threat-intel')} />
      <KpiTile label="Endpoints" value={fmtNum(kpis.endpoints)} hint={`${fmtNum(coverage.endpoints_resolved_to_vm)} resolved to cloud VMs`} icon={<Laptop size={13} />} />
    </div>
  )
}

...
export function AlertsTable({ alerts, columns, sort, onSortChange, serverSort, onRowClick, selectedId, className, emptyTitle, actorNames }: Props) {
  ...
  const all: Record<AlertColumnKey, Column<AlertSummary>> = {
    ...
    asset: {
      key: 'asset', header: 'Asset', width: '170px',
      render: (a) => (
    ...
    source: { key: 'source', header: 'Source', width: '80px', render: (a) => <SourceChip source={a.source_system} />, sortValue: (a) => a.source_system },
    vendor: { key: 'vendor', header: 'Vendor', width: '86px', render: (a) => <SeverityChip severity={a.vendor_severity} />, sortValue: (a) => a.vendor_severity_rank },
    contextual: {
      key: 'contextual', header: 'Contextual', width: '104px',
      render: (a) => (
    ...
    storyline: { key: 'storyline', header: 'Storyline', width: '100px', render: (a) => <StorylineChip id={a.storyline_id} onClick={a.storyline_id ? () => navigate(`/storylines/${encodeURIComponent(a.storyline_id!)}`) : undefined} />, sortValue: (a) => a.storyline_id ?? '' },
    ti: { key: 'ti', header: 'Threat intel', width: '132px', render: (a) => <TIBadge alert={a} actorNames={actorNames} />, sortValue: (a) => a.ioc_match_count + a.ti_actor_ids.length * 10 },
    time: { key: 'time', header: 'Detected', width: '120px', render: (a) => <span className="mono text-[11px] text-fg-2 whitespace-nowrap">{fmtTime(a.detected_at)}</span>, sortValue: (a) => a.detected_at ?? '' },
  }

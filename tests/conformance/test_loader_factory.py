...
def _settings(**overrides):
    ...
    ns.model_fields_set = set(overrides)  # mimic pydantic-settings: only explicitly provided fields
    return ns
...
def test_factory_networkx_and_fallbacks(mini_graph: ContextGraph, tmp_path: Path) -> None:
    assert isinstance(make_store(_settings(graph_backend="networkx"), mini_graph), NetworkXStore)
    # missing database -> networkx fallback even when the backend was explicit
